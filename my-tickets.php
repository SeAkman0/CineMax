...
// =======================================================
//  2. VERİTABANI SORGUSU (BİLET LİSTESİ)
// =======================================================

// Kullanıcının aldığı biletleri çekmek için karmaşık bir JOIN sorgusu yazıyoruz.
// Amacımız: Bilet tablosundan Film, Seans ve Salon bilgilerine erişmek.
// Ayrıca filmin aktiflik durumu (is_active) ve salon adı gibi detayları da alıyoruz.
$sql = "SELECT t.*, s.start_time, f.title, f.poster_url, f.is_active, h.name as hall_name 
        FROM tickets t
        JOIN sessions s ON t.session_id = s.id
        JOIN films f ON s.film_id = f.id
        JOIN halls h ON s.hall_id = h.id
        WHERE t.user_id = ?        -- Sadece bu kullanıcının biletleri
        ORDER BY t.id DESC";       -- En son alınan bilet en üstte görünsün

$stmt = $pdo->prepare($sql);
$stmt->execute([$user_id]);
$tickets = $stmt->fetchAll(); // Sonuçları bir dizi (array) olarak al.
?>
...
<div class="container" style="padding: 60px 20px;">
    ...
    <?php else: ?>
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: 30px;">
            ...
            <?php 
                // ==========================================
                //  4. DURUM YÖNETİMİ (0-1-2-3 MANTIĞI)
                // ==========================================
                
                $simdi = time(); // Şu anki zaman
                $seans_zamani = strtotime($ticket['start_time']); // Seans zamanı
                $durum_kodu = 0; // Varsayılan: Aktif (0)

                // Öncelik Sırasına Göre Kontrol:
                
                // 1. Film Vizyondan Kalktı mı? (En yüksek öncelik)
                if ($ticket['is_active'] == 0) {
                    $durum_kodu = 2; // Vizyondan Kalktı
                } 
                // 2. Bilet Kullanıldı mı? (Turnikeden geçti mi?)
                elseif ($ticket['is_used'] == 1) {
                    $durum_kodu = 1; // Kullanıldı
                } 
                // 3. Seans Saati Geçti mi?
                elseif ($seans_zamani < $simdi) {
                    $durum_kodu = 3; // Süresi Doldu
                } 
                // 4. Hiçbiri değilse Aktiftir.
                else {
                    $durum_kodu = 0; // Aktif
                }

                // --- TASARIM AYARLARI (RENKLER VE STİLLER) ---
                switch ($durum_kodu) {
                    case 2: // Kırmızı: İptal / Vizyondan Kalktı
                        $badge_bg = "#e74c3c"; 
                        $badge_text = "VİZYONDAN KALKTI";
                        $card_style = "opacity: 0.6; background: #fbeaea; border: 1px solid #e74c3c;";
                        $qr_style = "display:none;"; // QR Kodunu gizle (Kullanılamaz)
                        break;
                    
                    case 3: // Turuncu: Zamanı Geçti
                        $badge_bg = "#e67e22";
                        $badge_text = "SÜRESİ DOLDU";
                        $card_style = "opacity: 0.7; background: #fdf2e9; border: 1px solid #d35400;";
                        $qr_style = "filter: grayscale(100%); opacity: 0.2;"; // QR silik
                        break;
                    
                    case 1: // Gri: Kullanıldı
                        $badge_bg = "#7f8c8d";
                        $badge_text = "KULLANILDI";
                        $card_style = "opacity: 0.8; background: #f4f6f9; border: 1px solid #ccc;";
                        $qr_style = "filter: grayscale(100%); opacity: 0.3;"; // QR silik
                        break;
                        ...
                        <div style="margin-top: 15px; font-weight: bold; color: #333; display:flex; justify-content:space-between; align-items:center;">
                            ...
                            <?php if($durum_kodu == 0): ?>
                                <form action="cancel-ticket.php" method="POST" onsubmit="return confirm('Silmek istediğinize emin misiniz?');" style="margin:0;">
                                    ...
                                    <button type="submit" style="background:none; border:none; color:#ff4757; cursor:pointer; font-size:1.2rem;" title="İptal Et">
